...
class MenuView(arcade.View):
    ...
    def on_draw(self):
        ...
        if self.current_state == MAIN_MENU:
            arcade.draw_text("Мышата", SCREEN_WIDTH // 2, SCREEN_HEIGHT - 100,
    ...
    def draw_header(self, text):
        arcade.draw_text(text, SCREEN_WIDTH // 2, SCREEN_HEIGHT - 80,
    ...
    def draw_menu_item(self, text, index):
        y = SCREEN_HEIGHT - 220 - index * 50
        arcade.draw_text(f"{text}", SCREEN_WIDTH // 2, y,
                         arcade.color.DARK_GRAY, font_size=24, anchor_x="center")

    def draw_back_button(self):
        arcade.draw_text("Назад", 80, 60, arcade.color.BLUE, font_size=18)

    def on_mouse_press(self, x: float, y: float, button: int, modifiers: int):
        if self.current_state == MAIN_MENU:
            for i, _ in enumerate(["К уровням", "Правила", "Управление"]):
                btn_y = SCREEN_HEIGHT - 220 - i * 50
